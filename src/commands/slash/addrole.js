...
module.exports = class AddRoleSlashCommand extends SlashCommand {
	...
	async run(interaction) {
			...
			return await interaction.editReply({
				embeds: [
					new ExtendedEmbedBuilder({
						...
						.setTitle(getMessage('commands.slash.addrole.not_staff.title'))
						.setDescription(getMessage('commands.slash.addrole.not_staff.description')),
				],
			});
		}
		...
		await ticketChannel.permissionOverwrites.edit(
			...
			`${interaction.user.tag} added role ${role.name} to the ticket`,
		);
		...
		await ticketChannel.send({
			embeds: [
				new ExtendedEmbedBuilder()
					...
					.setDescription(getMessage('commands.slash.addrole.added', {
						added: role.toString(),
		...
		await interaction.editReply({
			embeds: [
				new ExtendedEmbedBuilder({
					...
					.setTitle(getMessage('commands.slash.addrole.success.title'))
					.setDescription(getMessage('commands.slash.addrole.success.description', {
						role: role.toString(),
		...
		logTicketEvent(this.client, {
			...
			diff: {
				original: {},
				updated: { [getMessage('log.ticket.added')]: role.name },
			},
			target: {
				id: ticket.id,
				name: `<#${ticket.id}>`,
